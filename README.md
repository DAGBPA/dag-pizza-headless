# Headless wallet for DAG Pizza network

This is a headless alternative of the [GUI wallet](../../../dag-pizza) for DAG Pizza network.  It is designed for an always-online deployment on a server.
...
npm install
```
If you want to accept incoming connections, you'll need to set up a proxy, such as nginx, to forward all websocket connections on a specific path to your daemon running this code.  See example configuration for nginx in [dag-pizza-dough](../../../dag-pizza-dough) documentation.

## Run
```sh
node start.js
```
The first time you run it, it will generate a new extended private key (BIP44) and ask you for a passphrase to encrypt it.  The BIP39 mnemonic will be saved to the file keys.json in the app data directory (see [dag-pizza-dough](../../../dag-pizza-dough) for its location), the passphrase is, of course, never saved.  Every time you start the wallet, you'll have to type the passphrase.  One implication of this is the wallet cannot be started automatically when your server restarts, you'll have to ssh the server and type the passphrase.
...
* `pay <amount in pizza>` to request withdrawal from the headless wallet to your `payout_address`.
...
Use TOR ([conf.socksHost, conf.socksPort, and conf.socksLocalDNS](../../../dag-pizza-dough#confsockshost-confsocksport-and-confsockslocaldns)) to hide your server IP address from potential attackers.
